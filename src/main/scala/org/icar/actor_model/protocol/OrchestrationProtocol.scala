...
package object OrchestrationProtocol extends Protocol {
	...
	trait ProcessCommitment {
		...
		def command_task(task:SolutionTask) : ProtocolPart = RequestConcreteExecution(id,task)
	}
	...
	case class RequestConcreteExecution private(id:Long, task:SolutionTask) extends ProtocolPart {
		def progress_task : ProtocolPart = InformProgress(id,task)
		def concrete_failure : ProtocolPart = InformConcreteFailure(id,task)
	}
	case class PauseConcrete private(id:Long, task:SolutionTask) extends ProtocolPart
	case class RestartConcrete private(id:Long, task:SolutionTask) extends ProtocolPart

	case class InformProgress private(id:Long,task:SolutionTask) extends ProtocolPart
	case class InformConcreteFailure private(id:Long,task:SolutionTask) extends ProtocolPart
