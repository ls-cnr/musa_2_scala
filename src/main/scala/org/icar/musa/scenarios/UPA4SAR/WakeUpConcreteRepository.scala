...
class CheckWakeUp1(abs_cap : GroundedAbstractCapability) extends ConcreteCapability("check_wakeup_1",abs_cap) {
  ...
  override def execute(w : StateOfWorld,in:Measurables): Unit = { println("executing CheckWakeUp"); if (count<2) set_scenario("over_sleeping") else set_scenario("anomaly") }
...
class RemindWakeUp1(abs_cap : GroundedAbstractCapability) extends ConcreteCapability("remind_wakeup_1",abs_cap) {
  ...
  override def execute(w : StateOfWorld,in:Measurables): Unit = { println("executing RemindWakeUp"); set_scenario("remind") }
...
class AlertAnomaly1(abs_cap : GroundedAbstractCapability) extends ConcreteCapability("alert_anomaly_1",abs_cap) {
  ...
  override def execute(w : StateOfWorld,in:Measurables): Unit =    { println("executing AlertAnomaly"); set_scenario("alert_cargiver") }
