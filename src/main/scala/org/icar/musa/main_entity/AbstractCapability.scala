...
case class GroundedAbstractCapability(
                                       override val name:String,
                                       override val pre: FOLCondition,
                                       override val post: FOLCondition,
                                       scenarios : Map[String,EvolutionScenario],
                                       in:DataInSpecification = DataInSpecification(ArrayBuffer()),
                                       out:DataOutSpecification = DataOutSpecification(ArrayBuffer()),
                                       grounding:Map[VariableTerm,ConstantTerm]=Map()
                                     ) extends AbstractCapability(name,pre,post)
