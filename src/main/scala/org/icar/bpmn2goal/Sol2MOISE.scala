...
class Sol2MOISE(ontology: Domain, goals:List[GoalSPEC], yellowpage:List[ServiceDescr], initial : StateOfWorld) {
	...
	def apply_rule__group_roles(s: Solution): Elem = {
		<roles>
			<role id="manager" min="1" max="1"/>{s.wfitems.filter(i => i.isInstanceOf[scala.org.icar.high_level_specification.SolutionTask]).map(t =>
				<role id={t.asInstanceOf[scala.org.icar.high_level_specification.SolutionTask].grounding.capability.id + "_role"} min="1" max="1"/>
		)}
		</roles>
	}
