...
object ProvaSolver extends App {
  ...
  val evo_work = Array(
    EvolutionGrounding("base",Array[EvoOperator](
      AddEvoOperator(GroundPredicate("worked", AtomTerm("doc"))),AddEvoOperator(GroundPredicate("to_be_revised", AtomTerm("doc")))
    )))
  ...
  solver.iterate_until_termination(IterationTermination(4))
